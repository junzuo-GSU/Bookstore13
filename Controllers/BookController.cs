    ...
    public class BookController : Controller
    {
        ...
        public ViewResult List(BookGridData values)
        {
            ...
            var options = new QueryOptions<Book> { 
                Includes = "Authors, Genre",//for each book, associated authors and genre are required for display
                OrderByDirection = values.SortDirection,
                PageNumber = values.PageNumber,
                PageSize = values.PageSize
            };
            //books are sorted by default on title, but can also be sorted by genre and price
        ...
        public ViewResult Details(int id)
        {
            var book = data.Get(new QueryOptions<Book> {
                Where = b => b.BookId == id,
                Includes = "Authors, Genre" //for each book, we need info of authors and genre
            }) ?? new Book();
            return View(book);
        }
    }   
