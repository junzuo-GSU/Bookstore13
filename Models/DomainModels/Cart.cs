namespace Bookstore.Models
{
    ...
    public class Cart
    {
        ...
        public void Load(Repository<Book> data)
        {
            ...
            // if items mismatch between cookie and session, clean session.
        ...
        public CartItem? GetById(int? id) { //id is BookId
            if (items == null || id == null) {
                return null;
            } else {
                return items.FirstOrDefault(ci => ci.Book?.BookId == id);//this is not a DB lookup but a collection lookup
            }
        }
